...
export default function TournamentOperationsPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 sm:py-10"><TournamentAdminNav tournamentId={tournamentId} />
    ...
    <section className="grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-line p-5"><p className="text-xs uppercase tracking-wide text-ink-faint">Role</p><p className="mt-2 text-xl font-semibold text-ink">{role}</p><p className="mt-2 text-sm text-ink-faint">Event control: {canOperate ? "enabled" : "read only"}</p></div><div className="rounded-xl border border-line p-5"><p className="text-xs uppercase tracking-wide text-ink-faint">Spectator views</p><p className="mt-2 text-2xl font-semibold text-ink">{metrics?.totals.views ?? 0}</p></div><div className="rounded-xl border border-line p-5"><p className="text-xs uppercase tracking-wide text-ink-faint">Watch time</p><p className="mt-2 text-2xl font-semibold text-ink">{Math.floor((metrics?.totals.watchSeconds ?? 0) / 60)} min</p></div></section>
