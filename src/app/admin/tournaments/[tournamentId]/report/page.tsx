...
type TopMatch = { matchId: string; dayKey: string; views: number; watchSeconds: number; match: { round: string | null; playerOne: { gamertag: string } | null; playerTwo: { gamertag: string } | null; station: { label: string } | null } | null };
type StationSummary = { label: string; status: string; droppedFrames: number; bitrateKbps: number; lastHeartbeatAt: string | null };
type Report = { tournament: { name: string; game: string; status: string }; generatedAt: string; summary: { totalMatches: number; completed: number; live: number; queued: number; avgMatchMinutes: number; views: number; uniqueViewers: number; watchHours: number; auditEvents: number }; topMatches: TopMatch[]; stations: StationSummary[] };

type ReportCard = [string, number];
...
export default function TournamentReportPage() {
  ...
  useEffect(() => { fetch(`/api/tournaments/${tournamentId}/report`, { cache: "no-store" }).then(async r => { const j=await r.json(); if(!r.ok) throw new Error(j.error); setData(j as Report); }).catch(e=>setError(e instanceof Error ? e.message : "Unable to load report")); }, [tournamentId]);
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6"><TournamentAdminNav tournamentId={tournamentId} />
    ...
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{cards.map(([l,v])=><div key={l} className="rounded-card border border-arena-600 bg-arena-900 p-4"><p className="font-mono text-[10px] uppercase text-ink-faint">{l}</p><p className="mt-1 font-display text-2xl">{v}</p></div>)}</div>
    <section className="rounded-card border border-arena-600 bg-arena-900 p-4"><div className="flex items-end justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-ink-faint">Audience</p><h2 className="font-display text-xl uppercase">Top matches</h2></div><span className="font-mono text-[10px] text-ink-faint">Views / watch time</span></div><div className="mt-3 divide-y divide-arena-700">{(data.topMatches ?? []).map(m=><div key={`${m.matchId}-${m.dayKey}`} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="text-sm">{m.match ? `${m.match.playerOne?.gamertag ?? "TBD"} vs ${m.match.playerTwo?.gamertag ?? "TBD"}` : m.matchId}</p><p className="text-[10px] font-mono text-ink-faint">{m.match?.round ?? "Match"}{m.match?.station ? ` · ${m.match.station.label}` : ""}</p></div><div className="text-right font-mono text-xs"><span>{m.views} views</span><span className="ml-3 text-ink-faint">{Math.floor(m.watchSeconds / 60)} min</span></div></div>)}{(!data.topMatches || data.topMatches.length === 0) && <p className="py-4 text-sm text-ink-faint">No audience data yet.</p>}</div></section>
    <section className="rounded-card border border-arena-600 bg-arena-900 p-4"><h2 className="font-display text-xl uppercase">Station performance</h2><div className="mt-3 divide-y divide-arena-700">{data.stations.map(s=><div key={s.label} className="flex flex-wrap justify-between gap-2 py-3 text-sm"><span>{s.label}</span><span className="font-mono text-xs text-ink-faint">{s.status} · {s.bitrateKbps} kbps · {s.droppedFrames} dropped</span></div>)}</div></section>
