...
export default async function AdminPage() {
  ...
  if (!isPlatformAdmin && !isOrganizationAdmin && !isOrganizer) redirect("/dashboard");
  if (isOrganizer && !isPlatformAdmin) redirect("/organizer");
  ...
  const [users, teams, players] = await Promise.all([
    isPlatformAdmin ? db.user.count() : Promise.resolve(0),
    isPlatformAdmin ? db.team.count() : Promise.resolve(0),
    isPlatformAdmin ? db.player.count() : Promise.resolve(0),
  ]);
  ...
  const tools = [
    ...
    ...(isPlatformAdmin ? [["/organization/settings", "Organization", "Branding, members and organization controls."], ["/admin/users", "Users", "Platform user and role management."]] : []),
  ];

  return <main className="min-h-screen bg-arena-950 px-6 py-8"><div className="mx-auto max-w-7xl">
    <header className="mb-8 rounded-card border border-arena-700 bg-arena-900 p-5"><p className="font-mono text-xs uppercase tracking-widest text-signal-live">Admin workspace</p><h1 className="mt-1 font-display text-4xl uppercase">Competition control center</h1><p className="mt-2 max-w-3xl text-sm text-ink-muted">Create, operate, score, monitor and publish competitions from one place. Platform administration stays restricted while organization operators work within their own competition scope.</p></header>
