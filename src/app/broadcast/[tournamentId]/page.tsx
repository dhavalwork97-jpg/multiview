...
export default function BroadcastProductionConsole({ params }: BroadcastPageProps) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/broadcast/destinations?tournamentId=${encodeURIComponent(tournamentId)}`)
      .then((response) => response.ok ? response.json() : Promise.reject(new Error("Could not load destinations")))
      .then((data: { destinations: BroadcastDestination[] }) => setDestinations(data.destinations ?? []))
      .catch((error) => setMessage(error instanceof Error ? error.message : "Could not load destinations"));
  }, [tournamentId]);
  ...
  async function addDestination() {
    ...
    try {
      const response = await fetch("/api/broadcast/destinations", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ tournamentId, provider, label, channelName, streamUrl }),
      });
    ...
    <main style={{ minHeight: "100vh", background: "#07080d", color: "#f7f8ff", padding: 32, fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "1.35fr .65fr", gap: 18 }}>
          <div style={{ border: "1px solid #222631", borderRadius: 20, background: "linear-gradient(145deg,#10131b,#090a10)", padding: 22 }}>
            ...
            <div style={{ aspectRatio: "16/9", borderRadius: 14, border: "1px solid #292d39", display: "grid", placeItems: "center", background: "radial-gradient(circle at 50% 40%,#24183b 0,#0b0c12 55%)", overflow: "hidden" }}>
              <div style={{ textAlign: "center" }}><div style={{ fontSize: 12, letterSpacing: ".2em", opacity: .45 }}>FGC LIVE</div><div style={{ fontSize: 52, fontWeight: 800, textTransform: "uppercase", marginTop: 8 }}>{active.replaceAll("-", " ")}</div><div style={{ marginTop: 12, opacity: .5, fontSize: 13 }}>OBS Browser Source · Realtime</div></div>
            </div>
          </div>
          <aside style={{ display: "grid", gap: 12 }}>
            {groups.map((group, index) => <div key={index} style={{ border: "1px solid #222631", borderRadius: 20, background: "#0c0e14", padding: 14, display: "grid", gap: 10 }}>{group.map((item) => <button key={item.scene} disabled={busy || !tournamentId} onClick={() => send(item.scene, item.type)} style={{ textAlign: "left", border: active === item.scene ? "1px solid #8b5cf6" : "1px solid #242936", borderRadius: 12, padding: "13px 14px", background: active === item.scene ? "#171127" : "#101219", color: "#fff", cursor: busy ? "wait" : "pointer" }}><div style={{ fontWeight: 700 }}>{item.label}</div><div style={{ fontSize: 11, opacity: .45, marginTop: 3 }}>{item.type}</div></button>)}</div>)}
          </aside>
        </section>

        <section style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {["Program", "Preview", "OBS Sync"].map((label, i) => <div key={label} style={{ border: "1px solid #222631", borderRadius: 16, background: "#0c0e14", padding: 16 }}><div style={{ fontSize: 11, opacity: .45, textTransform: "uppercase", letterSpacing: ".12em" }}>{label}</div><div style={{ marginTop: 8, fontSize: 20, fontWeight: 750 }}>{i === 0 ? active : i === 1 ? "Standby" : "Connected"}</div></div>)}
        </section>
        ...
        <section style={{ marginTop: 18, border: "1px solid #222631", borderRadius: 20, background: "#0c0e14", padding: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "150px 1fr 1fr 1.5fr auto", gap: 8 }}>
            <select value={provider} onChange={(event) => setProvider(event.target.value as BroadcastDestinationProvider)} style={fieldStyle}><option value="youtube">YouTube</option><option value="twitch">Twitch</option><option value="rtmp">Custom RTMP</option></select>
            <input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Destination label" style={fieldStyle} />
            <input value={channelName} onChange={(event) => setChannelName(event.target.value)} placeholder="Channel name (optional)" style={fieldStyle} />
            <input value={streamUrl} onChange={(event) => setStreamUrl(event.target.value)} placeholder={provider === "rtmp" ? "rtmp://…" : "Stream URL (optional until connected)"} style={fieldStyle} />
            <button disabled={savingDestination || !label.trim()} onClick={addDestination} style={{ border: "1px solid #8b5cf6", borderRadius: 10, padding: "0 16px", background: "#171127", color: "#fff", fontWeight: 700, cursor: savingDestination ? "wait" : "pointer" }}>{savingDestination ? "Saving…" : "Add"}</button>
          </div>
          <div style={{ marginTop: 10, fontSize: 12, opacity: .45 }}>Destinations are tournament-scoped. Provider OAuth and live-event creation are the next integration layer; this screen does not fake a connected provider.</div>
        </section>
      </div>
    </main>
  );
