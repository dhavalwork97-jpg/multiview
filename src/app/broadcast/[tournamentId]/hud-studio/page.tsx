...
export default function HudStudioPage({ params }: { params: { tournamentId: string } }) {
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 380px", gap: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 14 }}>
            {filtered.map((pkg) => {
              ...
              return <button key={pkg.id} onClick={() => setSelected(pkg.id)} style={{ textAlign: "left", minHeight: 190, border: active ? `1px solid ${pkg.accent}` : "1px solid #202731", background: "#0d1015", borderRadius: 16, padding: 18, color: "inherit", cursor: "pointer", boxShadow: active ? `0 0 0 1px ${pkg.accent}22, inset 0 0 50px ${pkg.accent}10` : "none" }}>
                <div style={{ height: 82, borderRadius: 11, marginBottom: 16, background: `linear-gradient(135deg, ${pkg.accent} 0%, #10131a 48%, ${pkg.accent2} 140%)`, position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", inset: 12, border: "1px solid #ffffff44", borderRadius: 8 }} />
                  <div style={{ position: "absolute", left: 14, bottom: 10, fontSize: 10, letterSpacing: ".16em", fontWeight: 900 }}>FGC BROADCAST</div>
                  <div style={{ position: "absolute", right: 14, top: 12, fontFamily: "monospace", fontWeight: 900 }}>P1  2 — 1  P2</div>
                </div>
                <div style={{ fontSize: 16, fontWeight: 850 }}>{pkg.name}</div>
                <div style={{ color: "#737d8b", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", marginTop: 4 }}>{pkg.family} · {pkg.games[0]}</div>
                <p style={{ color: "#8f98a5", fontSize: 12, lineHeight: 1.45, margin: "10px 0 0" }}>{pkg.description}</p>
              </button>;
            })}
          </div>
          ...
          <aside style={{ border: "1px solid #242b34", background: "#0d1015", borderRadius: 18, padding: 20, height: "fit-content", position: "sticky", top: 20 }}>
            <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".15em", color: selectedPackage.accent, fontWeight: 900 }}>Selected package</div>
            <h2 style={{ margin: "7px 0 5px", fontSize: 25 }}>{selectedPackage.name}</h2>
            <p style={{ color: "#8f98a5", fontSize: 13, lineHeight: 1.5 }}>{selectedPackage.description}</p>
