...
export default function BroadcastControlRoom({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  useEffect(() => {
    ...
    const onBroadcastUpdated = (event: { tournamentId?: string; scene?: string; commandType?: string }) => {
  ...
  useEffect(() => {
    if (!timelineRunning || timelineStartedAt === null) return;
    const timer = window.setInterval(() => setElapsedMs(Date.now() - timelineStartedAt), 100);
    return () => window.clearInterval(timer);
  }, [timelineRunning, timelineStartedAt]);
  ...
  async function issueCommand(scene: BroadcastScene, type: BroadcastCommandType = "SCENE_SET") {
    ...
    try {
      const response = await fetch("/api/broadcast/command", {
        ...
        body: JSON.stringify({ tournamentId, type, scene, obsMapping }),
      });
  ...
  async function sendCue() {
    if (!nextCue || !tournamentId) return;
    const ok = await issueCommand(nextCue.command.scene, nextCue.command.type);
    if (ok) setCueIndex((index) => Math.min(index + 1, DEFAULT_MATCH_TIMELINE.cues.length - 1));
  }

  function startTimelineClock() {
    setTimelineRunning(true);
    setTimelineStartedAt(Date.now() - elapsedMs);
    setStatus("LIVE");
  }
  ...
  function skipCue() {
    if (nextCue) setCueIndex((index) => Math.min(index + 1, DEFAULT_MATCH_TIMELINE.cues.length - 1));
  }
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            ...
            <div style={{ marginTop: 7, fontSize: 12, opacity: .5 }}>MANUAL PROGRAM CONTROL · AUTOMATION ASSISTS, NEVER TAKES OVER</div>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center", fontSize: 12 }}>
            <a href={`/broadcast/${tournamentId}/control-room/obs`} style={link}>OBS MAPPING</a>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "1.4fr .9fr", gap: 18, marginBottom: 18 }}>
          <div style={panel}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {!timelineRunning ? <button onClick={startTimelineClock} style={button(false)}>START CLOCK</button> : <button onClick={pauseTimelineClock} style={button(false)}>PAUSE CLOCK</button>}
              <button onClick={resetTimeline} style={button(false)}>RESET</button>
              <span style={{ marginLeft: "auto", fontSize: 12, opacity: .5 }}>{formatElapsed(elapsedMs)}</span>
            </div>
          </div>

          <div style={panel}>
            <div style={label}>PREVIEW SCENE · OPERATOR SELECTED</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
              {scenes.map((scene) => <button key={scene} onClick={() => setPreview(scene)} style={{ ...button(preview === scene), padding: "10px 8px", fontSize: 11 }}>{scene.replace("-", " ").toUpperCase()}</button>)}
            </div>
          </div>
        </section>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 18 }}>
          ...
          <Card title="OBS TARGET" value={obsMapping[program].toUpperCase()} />
        ...
        <section style={panel}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            <div>
              <div style={label}>PRODUCTION ASSIST</div>
              <h2 style={{ margin: "7px 0 0", fontSize: 20 }}>Cue Timeline</h2>
              <p style={{ margin: "5px 0 0", fontSize: 12, opacity: .5 }}>The clock and cues never change Program automatically. The operator must send each cue.</p>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={skipCue} disabled={!nextCue} style={{ ...button(false), opacity: nextCue ? 1 : .4 }}>SKIP CUE</button>
              <button onClick={sendCue} disabled={!nextCue} style={{ ...button(true), opacity: nextCue ? 1 : .4 }}>SEND NEXT CUE</button>
            </div>
          </div>
          <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
            {DEFAULT_MATCH_TIMELINE.cues.map((cue, index) => {
              const active = index === cueIndex;
              const sent = index < cueIndex;
              return <div key={cue.id} style={{ padding: 12, borderRadius: 12, border: `1px solid ${active ? "#7c3aed" : "#252936"}`, background: active ? "#151021" : "#0a0c11", opacity: sent ? .45 : 1 }}>
                <div style={{ fontSize: 10, opacity: .45 }}>{String(cue.atMs / 1000).padStart(4, "0")}s</div>
                <div style={{ marginTop: 6, fontWeight: 800, fontSize: 12 }}>{cue.id.toUpperCase()}</div>
                <div style={{ marginTop: 4, fontSize: 10, opacity: .5 }}>OBS · {obsMapping[cue.command.scene].toUpperCase()}</div>
              </div>;
            })}
          </div>
          {currentCue && <div style={{ marginTop: 12, fontSize: 11, opacity: .55 }}>LAST CUE · {currentCue.id.toUpperCase()} · {currentCue.command.type}</div>}
        </section>
      </div>
    </main>
  );
...
function Monitor({ title, scene, live }: { title: string; scene: BroadcastScene; live?: boolean }) {
  return <div style={{ aspectRatio: "16 / 9", borderRadius: 14, background: "linear-gradient(135deg,#151822,#090a0f)", border: `1px solid ${live ? "#6d28d9" : "#252936"}`, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 16 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, letterSpacing: ".14em", opacity: .55 }}><span>{title}</span>{live && <span>ON AIR</span>}</div><div><div style={{ fontSize: 28, fontWeight: 800 }}>{scene.replace("-", " ").toUpperCase()}</div><div style={{ opacity: .45, fontSize: 11, marginTop: 4 }}>FGC NATIVE BROADCAST GRAPHICS</div></div></div>;
}
