...
const key = (id: string) => `fgc-broadcast-overlay-builder:${id}`;
...
export default function OverlayBuilderPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  useEffect(() => {
    void params.then(({ tournamentId: id }) => {
      ...
      try {
        ...
        if (raw) setConfig(normalizeOverlayConfig(JSON.parse(raw)));
        else setConfig(normalizeOverlayConfig({ themeId: getBroadcastTheme(gameValue).id }));
      } catch { setConfig(normalizeOverlayConfig({ themeId: getBroadcastTheme(gameValue).id })); }
    });
  }, [params]);
  ...
  const updateElement = (kind: OverlayElementKind, next: Record<string, unknown>) => {
    setConfig((current) => normalizeOverlayConfig({ ...current, elements: current.elements.map((item) => item.kind === kind ? { ...item, ...next } : item) }));
    setSaved(false);
  };
  function save() { if (!tournamentId) return; window.localStorage.setItem(key(tournamentId), JSON.stringify(config)); setSaved(true); }
  function reset() { const next = normalizeOverlayConfig({ themeId: getBroadcastTheme(game).id }); setConfig(next); if (tournamentId) window.localStorage.removeItem(key(tournamentId)); setSaved(false); }
  ...
  return <main style={{ minHeight: "100vh", background: "#07080d", color: "#f7f8ff", padding: 28, fontFamily: "Inter,system-ui,sans-serif" }}><div style={{ maxWidth: 1280, margin: "0 auto" }}>
    <header style={header}><div><div style={eyebrow}>FGC BROADCAST STUDIO · V33</div><h1 style={title}>Overlay Builder</h1><p style={muted}>Build the broadcast look without external graphics software. Changes are previewed live in the production canvas.</p></div><a href={`/broadcast/${tournamentId}/control-room`} style={link}>← CONTROL ROOM</a></header>
    <div style={layout}>
      <aside style={panel}>
        <div style={eyebrow}>THEME</div><h2 style={sectionTitle}>Visual system</h2>
        <label style={label}>GAME THEME<select value={config.themeId} onChange={(e) => patch({ themeId: e.target.value as BroadcastThemeId })} style={input}>{Object.values(BROADCAST_THEMES).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        <label style={label}>ACCENT<input value={config.customAccent ?? theme.accent} onChange={(e) => patch({ customAccent: e.target.value })} style={input} /></label>
        <label style={label}>SECONDARY<input value={config.customAccentAlt ?? theme.accentAlt} onChange={(e) => patch({ customAccentAlt: e.target.value })} style={input} /></label>
        ...
        <div style={{ ...eyebrow, marginTop: 24 }}>ELEMENTS</div>
        <div style={{ display: "grid", gap: 9, marginTop: 10 }}>{kinds.map((kind) => { const item = config.elements.find((entry) => entry.kind === kind)!; return <div key={kind} style={controlRow}><label style={{ flex: 1, fontSize: 12, fontWeight: 800, textTransform: "uppercase" as const }}><input type="checkbox" checked={item.enabled} onChange={(e) => updateElement(kind, { enabled: e.target.checked })} /> {kind.replace("-", " ")}</label><select value={item.position} onChange={(e) => updateElement(kind, { position: e.target.value })} style={miniInput}>{positions.map((position) => <option key={position}>{position}</option>)}</select></div>})}</div>
        <div style={{ marginTop: 20, display: "flex", gap: 8 }}><button onClick={save} style={button}>SAVE</button><button onClick={reset} style={secondary}>RESET</button>{saved && <span style={savedText}>Saved</span>}</div>
      </aside>
...
function Preview({ config, theme }: { config: BroadcastOverlayConfig; theme: ReturnType<typeof getBroadcastTheme> }) {
  ...
  return <div style={{ ...canvas, ...vars, fontFamily: config.fontFamily }}>
    <div style={glow(theme.glow)} />
    ...
    {enabled("sponsor")?.enabled && config.sponsor.enabled && <div style={{ ...sponsor, ...pos(config.sponsor.position) }}><small>{config.sponsor.label}</small><strong>{config.sponsor.logoUrl ? "SPONSOR LOGO" : "YOUR SPONSOR"}</strong></div>}
...
const scoreboard = { position: "absolute", display: "grid", gridTemplateColumns: "1fr auto auto auto 1fr", alignItems: "center", gap: 14, padding: "12px 18px", minWidth: "72%", background: "var(--s)", border: "1px solid var(--a)", boxShadow: "0 18px 55px color-mix(in srgb,var(--a) 30%,transparent)", borderRadius: 8 } as React.CSSProperties;
...
const sponsor = { position: "absolute", display: "grid", gap: 3, padding: "10px 14px", background: "var(--s)", borderRight: "3px solid var(--b)" } as React.CSSProperties;
