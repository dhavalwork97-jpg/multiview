...
export default function ObsSceneMappingPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  useEffect(() => { void params.then(({ tournamentId: id }) => { setTournamentId(id); try { const raw = window.localStorage.getItem(`${storagePrefix}${id}`); if (raw) setMapping(normalizeObsSceneMapping(JSON.parse(raw) as Partial<ObsSceneMapping>)); } catch { /* safe defaults */ } }); }, [params]);
  function update(scene: BroadcastScene, value: string) { setMapping((current) => ({ ...current, [scene]: value })); setSaved(false); }
  function save() { if (!tournamentId) return; window.localStorage.setItem(`${storagePrefix}${tournamentId}`, JSON.stringify(mapping)); setSaved(true); }
  function reset() { setMapping(DEFAULT_OBS_SCENE_MAPPING); if (tournamentId) window.localStorage.removeItem(`${storagePrefix}${tournamentId}`); setSaved(false); }
