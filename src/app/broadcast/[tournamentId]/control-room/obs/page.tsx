...
export default function ObsSceneMappingPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  function update(scene: BroadcastScene, value: string) {
    setMapping((current) => ({ ...current, [scene]: value as BroadcastScene }));
    setSaved(false);
  }
  ...
  function reset() {
    ...
    if (tournamentId) window.localStorage.removeItem(`${storagePrefix}${tournamentId}`);
    setSaved(false);
  }
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "flex-end", marginBottom: 24 }}>
          <div>
            ...
            <h1 style={{ fontSize: 34, margin: "7px 0 0" }}>OBS Scene Mapping</h1>
            <p style={{ margin: "8px 0 0", opacity: .55, fontSize: 13 }}>Map every FGC production scene to the exact OBS scene name used by your production setup.</p>
          </div>
        ...
        <section style={panel}>
          <div style={gridHeader}>
            <span>FGC SCENE</span><span>OBS SCENE</span>
          </div>
          {scenes.map((scene) => (
            <div key={scene} style={row}>
              <div>
                <strong>{scene.replace("-", " ").toUpperCase()}</strong>
                <small>Browser Source runtime event</small>
              </div>
              <input value={mapping[scene]} onChange={(event) => update(scene, event.target.value)} aria-label={`OBS scene for ${scene}`} style={input} />
            </div>
          ))}
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 18 }}>
            <button onClick={save} style={button(true)}>SAVE MAPPING</button>
            <button onClick={reset} style={button(false)}>RESET DEFAULTS</button>
            {saved && <span style={{ fontSize: 12, color: "#22c55e" }}>Saved for this tournament</span>}
          </div>
        </section>

        <section style={{ ...panel, marginTop: 14 }}>
          <div style={label}>HOW IT WORKS</div>
          <div style={{ marginTop: 10, display: "grid", gap: 8, fontSize: 13, opacity: .7 }}>
            <div>1. Match each FGC scene to the exact scene name in OBS.</div>
            <div>2. Use TAKE PREVIEW or the production timeline from the control room.</div>
            <div>3. FGC includes the resolved <code>obsScene</code> in realtime broadcast events and audit payloads.</div>
          </div>
        </section>
      </div>
    </main>
