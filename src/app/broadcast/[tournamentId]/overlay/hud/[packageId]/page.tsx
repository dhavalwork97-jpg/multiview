...
export default function HudOutput({ params }: { params: { tournamentId: string; packageId: string } }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/hud/${encodeURIComponent(params.tournamentId)}${query}`, { cache: "no-store" });
  ...
  return (
    <div className="fgc-overlay-root" style={{ position: "fixed", inset: 0, pointerEvents: "none", background: "transparent", color: "white", fontFamily: "Inter, Arial, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: 34, right: 48, padding: "7px 13px", background: "rgba(7,9,13,.82)", border: "1px solid #29303a", fontSize: 9, color: "#9ca3af", letterSpacing: ".1em" }}>{match?.game?.toUpperCase() ?? "FGC"} · {params.tournamentId.slice(0, 8).toUpperCase()}</div>
    </div>
  );
}
