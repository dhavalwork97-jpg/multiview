...
import { applyBroadcastOverlayEvent, createInitialOverlayState, getOverlayThemeVars, readNumber, readString, type BroadcastOverlayEvent, type BroadcastOverlayState } from "@/lib/broadcast/overlay-runtime";
...
export default function AnimatedBroadcastOverlay({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  const winner = readString(overlay?.winner ?? overlay?.teamName, teamA);
  const eventType = readString(overlay?.eventType);
  const playerName = readString(overlay?.playerName, "MVP");
  const playerTag = readString(overlay?.playerTag, "PLAYER");
  const placement = Math.max(1, Math.floor(readNumber(overlay?.placement, 1)));
  ...
  const isMvp = current.scene === "winner" && eventType === "mvp";
  ...
  return (
    <main className={`stage anim-${animation}`} style={getOverlayThemeVars(theme)}>
      ...
      {isMvp && <section className="winner mvp"><div className="burst" /><small>MATCH MVP</small><span className="mvp-tag">{playerTag}</span><h1>{playerName}</h1><strong>{readString(overlay?.teamName, "TEAM")}</strong></section>}
      {current.scene === "winner" && !isMvp && <section className="winner"><div className="burst" /><small>MATCH WINNER</small><h1>{winner}</h1><span>{scoreA} — {scoreB}</span></section>}
      {current.scene === "champion" && <section className="winner champion"><div className="burst" /><small>TOURNAMENT CHAMPION</small><span className="place">#{placement}</span><h1>{winner}</h1><span>{title}</span></section>}
