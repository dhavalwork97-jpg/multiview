  ...
  return (
    <main className="page-shell overflow-hidden">
      <div className="page-container max-w-7xl">
        ...
        <section className="ds-section">
          <div className="mb-5"><p className="page-kicker">04 / Competition calendar</p><h2 className="section-heading mt-1">Every game, one stage</h2><p className="page-subtitle mt-1">The showcase pulls from the same public tournament data used by the viewer product.</p></div>
        ...
        {live?.sponsors.length ? <section className="pb-10"><div className="surface-card p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="page-kicker">05 / Partners</p><h2 className="section-heading mt-1 text-2xl">Powered by the ecosystem</h2></div><span className="font-mono text-[9px] uppercase tracking-widest text-ink-faint">Official event partners</span></div><div className="mt-5 flex flex-wrap gap-3">{live.sponsors.map((s) => <div key={s.id} className="rounded-card border border-arena-700 bg-arena-950/60 px-5 py-3">{s.logoUrl ? <img src={s.logoUrl} alt={s.name} className="h-7 max-w-32 object-contain" /> : <span className="font-display uppercase">{s.name}</span>}</div>)}</div></div></section> : null}
      </div>
    </main>
  );
}
