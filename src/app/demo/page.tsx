...
const productRoutes = [
  ["/admin", "Admin", "Production admin workspace"],
  ["/dashboard", "Dashboard", "Signed-in home"],
  ["/organizer", "Organizer", "Organizer command center"],
  ["/tournaments", "Tournaments", "Tournament directory"],
  ["/teams", "Teams", "Team directory"],
  ["/players", "Players", "Player directory"],
  ["/matches", "Matches", "Match operations"],
  ["/live", "Live", "Public live experience"],
  ["/broadcast", "Broadcast", "Broadcast workspace"],
  ["/multiview", "MultiView", "Production monitoring"],
  ["/overlay", "Overlay", "Live graphics"],
  ["/showcase", "Showcase", "Public championship showcase"],
] as const;

export default function DemoHome() {
  return <DemoShell title="The complete FGC platform" eyebrow="FGC Product Demo · Public Preview">
    <div className="grid gap-4 md:grid-cols-4">
      <Stat label="Live tournaments" value="07" detail="Production-ready"/>
      <Stat label="Players" value="128" detail="Registered"/>
      <Stat label="Concurrent viewers" value="12.4K" detail="Live broadcast"/>
      <Stat label="Stream health" value="99.9%" detail="All systems normal"/>
    </div>
    ...
    <div className="mt-10 space-y-10">
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-4">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.24em] text-ink-faint">Full product</p>
            <p className="mt-1 text-sm text-ink-muted">The demo is the safe public showcase. These links expose the corresponding production workspaces for authenticated review.</p>
          </div>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {productRoutes.map(([href, title, desc]) => <Link key={href} href={href} className="rounded-card border border-arena-800 bg-arena-950 p-4 transition hover:border-signal-live">
            <p className="font-display text-lg uppercase">{title}</p>
            <p className="mt-1 text-xs text-ink-muted">{desc}</p>
            <p className="mt-3 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Open product →</p>
          </Link>)}
        </div>
      </section>
    </div>
