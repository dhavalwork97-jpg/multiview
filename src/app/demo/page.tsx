...
export default async function DemoHome() {
  ...
  const links = [
    ...
    ["/tournaments", "Public Tournament Registry", "Viewer-facing tournament discovery and competition pages."],
    ["/teams", "Public Teams", "Viewer-facing teams and roster experience."],
    ["/players", "Public Players", "Viewer-facing player profiles and rankings."],
    ["/live", "Live", "Viewer-facing live competition experience."],
    ["/matches", "Matches", "Viewer-facing match listings and results."],
  ];
  return <main className="min-h-screen bg-arena-950 px-6 py-10 text-ink"><div className="mx-auto max-w-6xl"><p className="font-mono text-xs uppercase tracking-[0.3em] text-signal-live">FGC Product Demo</p><h1 className="mt-2 font-display text-5xl uppercase">Full platform walkthrough</h1><p className="mt-4 max-w-3xl text-ink-muted">Temporary public demo access for product video capture. No Clerk login is required in demo mode.</p>{tournament && <div className="mt-6 rounded-card border border-arena-700 bg-arena-900 p-4"><p className="text-xs uppercase text-ink-faint">Demo competition</p><p className="mt-1 font-display text-2xl uppercase">{tournament.name}</p><p className="text-sm text-ink-muted">{tournament.game} · {tournament.id}</p></div>}<div className="mt-8 grid gap-4 md:grid-cols-2">{links.map(([href,title,desc]) => <Link key={href} href={href} className="rounded-card border border-arena-700 bg-arena-900 p-5 transition hover:border-signal-live hover:bg-arena-800"><p className="font-display text-xl uppercase">{title}</p><p className="mt-2 text-sm text-ink-muted">{desc}</p><p className="mt-4 font-mono text-xs uppercase text-signal-live">Open demo →</p></Link>)}</div></div></main>;
}
