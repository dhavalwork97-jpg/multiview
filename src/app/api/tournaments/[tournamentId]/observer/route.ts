...
export async function GET(_req: Request, { params }: Params) {
  ...
  const state = await db.broadcastState.findUnique({
    where: { tournamentId },
    select: { matchId: true, overlay: true },
  });
  if (!state?.matchId) {
    return NextResponse.json({ matchId: null, observer: null, recommendations: [], isBattleRoyale: false });
  }
  ...
  if (!match) {
    return NextResponse.json({ matchId: state.matchId, observer: null, recommendations: [], isBattleRoyale: false });
  }
  ...
  const teams = match.sides.map((side) => {
    ...
    const kills = side.scoreEvents
      .filter((event) => /kill|elimination/i.test(event.metric))
  ...
  const autoFights = match.sides.flatMap((side) => {
    ...
    return [{
      id: `auto-${side.sideKey}`,
      teamKeys: [side.sideKey],
      intensity: /knock|fight/i.test(recent.metric) ? 90 : 70,
      label: `${teams.find((team) => team.key === side.sideKey)?.label ?? side.sideKey} recent combat`,
...
export async function POST(req: Request, { params }: Params) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid observer state", details: parsed.error.flatten() }, { status: 400 });
  }
  ...
  const current = await db.broadcastState.findUnique({
    where: { tournamentId },
    select: { overlay: true },
  });
