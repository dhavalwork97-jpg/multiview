...
export async function GET(_req: Request, { params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  const [matches, stations, audit] = await Promise.all([
  ...
  return NextResponse.json({ tournament, generatedAt: new Date().toISOString(), summary: { totalMatches: matches.length, completed: completed.length, live: live.length, queued: queued.length, avgMatchMinutes, auditEvents: audit }, stations: stationSummary });
}
