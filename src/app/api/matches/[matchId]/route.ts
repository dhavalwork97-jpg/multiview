...
export async function PATCH(req: Request, { params }: { params: Promise<{ matchId: string }> }) {
    ...
    await db.station.update({
      ...
      if (result.updated.status === "COMPLETED") {
        ...
        for (const downstream of advanced) {
          await publishEvent({ type: "bracket:advanced", tournamentId: result.updated.tournamentId, bracketId: result.updated.bracketId ?? "", matchId: downstream.targetMatchId, targetSideKey: downstream.targetSideKey });
        }
      }
  ...
  if (parsed.data.status === "COMPLETED") {
    if (existing.sides.length !== 2) {
      return NextResponse.json(
        { error: "Match requires exactly two sides." },
        { status: 409 },
      );
    }
  ...
  for (const next of advanced) {
    await publishEvent({
      ...
      matchId: next.targetMatchId,
      targetSideKey: next.targetSideKey,
    });
  }
}
  ...
  if (updated.status === "COMPLETED" && updated.stageId) {
    ...
    for (const downstream of advanced) {
      await publishEvent({ type: "bracket:advanced", tournamentId: updated.tournamentId, bracketId: updated.bracketId ?? "", matchId: downstream.targetMatchId, targetSideKey: downstream.targetSideKey });
    }
  }
