...
const participantSchema = z
  ...
  .superRefine((value, ctx) => {
    if (
      Number(Boolean(value.playerId)) +
        Number(Boolean(value.teamId)) !==
      1
    ) {
      ctx.addIssue({
        code: "custom",
        message:
          "Participant must contain exactly one playerId or teamId",
      });
    }
  });
  ...
  .superRefine((value, ctx) => {
    ...
    if (!value.playerOneId || !value.playerTwoId) {
      ctx.addIssue({
        code: "custom",
        message:
          "Provide sides for a generic match, or playerOneId/playerTwoId for a legacy match",
      });
    }
  });
...
export async function POST(req: Request) {
  ...
  try {
    await requireTournamentManage(parsed.data.tournamentId);
  } catch {
    return NextResponse.json(
      { error: "Forbidden" },
      { status: 403 },
  ...
  try {
    const match = await db.$transaction(
      async (tx) => {
          ...
          : [
              {
                ...
                participants: [
                  {
                    playerId: parsed.data.playerOneId!,
                  },
                ],
              },
              {
                ...
                participants: [
                  {
                    playerId: parsed.data.playerTwoId!,
                  },
                ],
              },
            ];
        ...
        /*
         * Generic V31 matches must reference participants that
         * actually belong to the tournament.
         */
          ...
          for (const side of sides) {
            for (const participant of side.participants) {
              if (participant.playerId) {
                const entrant =
                  await tx.tournamentEntrant.findUnique({
                    where: {
                ...
                if (!entrant) {
                  throw new Error(
                    `Player ${participant.playerId} is not registered in this tournament`,
                  );
                }
              }
              ...
              if (participant.teamId) {
                ...
                if (!tournamentTeam) {
                  throw new Error(
                    `Team ${participant.teamId} is not registered in this tournament`,
                  );
                }
              }
            }
        ...
        const created = await createGenericMatch(tx, {
          ...
          rules:
            (parsed.data.rules ??
              tournament.competitionRules) as
              | Record<string, unknown>
              | null,

          scoringAdapter:
            parsed.data.scoringAdapter,
         ...
         *
         * Only playerOne/playerTwo requests populate the
         * old Match columns.
         *
         * Generic Side A / Side B matches keep them NULL.
         */
        if (!parsed.data.sides) {
          return tx.match.update({
            ...
            data: {
              playerOneId:
                parsed.data.playerOneId,
              playerTwoId:
                parsed.data.playerTwoId,
            },
            include: {
              sides: {
                include: {
