...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    persistent: readBroadcastPersistentState(state?.overlay),
...
export async function PATCH(request: Request) {
  ...
  const state = await db.broadcastState.upsert({
    ...
    create: { tournamentId: body.tournamentId, scene: existing?.scene ?? "OFFLINE", overlay: persistent },
    update: { overlay: persistent },
    select: { scene: true, overlay: true, updatedAt: true },
  });

  return NextResponse.json({ scene: state.scene, persistent: readBroadcastPersistentState(state.overlay), updatedAt: state.updatedAt.toISOString() });
}
