...
export async function GET(request: Request) {
  ...
  if (!state) {
    return NextResponse.json({ state: { tournamentId, scene: "starting-soon", commandType: "PROGRAM_CLEAR", stationId: null, matchId: null, overlay: null, updatedAt: new Date(0).toISOString() } });
  }
  ...
  return NextResponse.json({
    state: {
      tournamentId,
      scene: state.scene,
      commandType: typeof overlay?.commandType === "string" ? overlay.commandType : "SCENE_SET",
