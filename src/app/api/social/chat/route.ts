...
const postSchema = z.object({ matchId: z.string().min(1).max(64), body: z.unknown(), parentId: z.string().min(1).max(64).nullable().optional() });
...
export async function GET(request: Request) {
  ...
  const rows = await db.$queryRaw<Array<{ id: string; matchId: string; parentId: string | null; body: string; status: string; createdAt: Date; userId: string; username: string; displayName: string | null; avatarUrl: string | null }>>`
    ...
    LIMIT 100
  `;
  return NextResponse.json({ messages: rows.reverse(), viewer: viewer ? { id: viewer.id, role: viewer.role } : null }, { headers });
}
...
export async function POST(request: Request) {
  ...
  if (parentId) {
    const parent = await db.$queryRaw<Array<{ id: string }>>`SELECT id FROM chat_messages WHERE id = ${parentId} AND match_id = ${match.id} AND status = 'ACTIVE' LIMIT 1`;
    if (!parent.length) return NextResponse.json({ error: "Reply target not found" }, { status: 400, headers });
  }
  const muted = await db.$queryRaw<Array<{ id: string }>>`SELECT id FROM chat_mutes WHERE match_id = ${match.id} AND user_id = ${user.id} AND expires_at > CURRENT_TIMESTAMP LIMIT 1`;
  if (muted.length) return NextResponse.json({ error: "You are temporarily muted in this chat" }, { status: 403, headers });
  const id = randomUUID();
  const createdAt = new Date();
  await db.$executeRaw`INSERT INTO chat_messages (id, match_id, user_id, parent_id, body, status, created_at, updated_at) VALUES (${id}, ${match.id}, ${user.id}, ${parentId}, ${body}, 'ACTIVE', ${createdAt}, ${createdAt})`;
  const message = { id, matchId: match.id, parentId, body, status: "ACTIVE", createdAt: createdAt.toISOString(), user: { id: user.id, username: user.username, displayName: user.displayName, avatarUrl: user.avatarUrl }, emotes: extractEmotes(body) };
...
async function reportMessage(userId: string, input: unknown) {
  ...
  const message = await db.$queryRaw<Array<{ id: string; matchId: string }>>`SELECT id, match_id AS "matchId" FROM chat_messages WHERE id = ${parsed.data.messageId} LIMIT 1`;
  if (!message.length) return NextResponse.json({ error: "Message not found" }, { status: 404, headers });
  try { await db.$executeRaw`INSERT INTO chat_reports (id, message_id, reporter_id, reason) VALUES (${randomUUID()}, ${parsed.data.messageId}, ${userId}, ${parsed.data.reason})`; } catch { return NextResponse.json({ error: "You already reported this message" }, { status: 409, headers }); }
...
async function moderateMessage(user: { id: string; role: string }, input: unknown) {
  ...
  const rows = await db.$queryRaw<Array<{ id: string; matchId: string; userId: string }>>`SELECT id, match_id AS "matchId", user_id AS "userId" FROM chat_messages WHERE id = ${parsed.data.messageId} LIMIT 1`;
  ...
  if (parsed.data.action === "mute") {
    ...
    await db.$executeRaw`INSERT INTO chat_mutes (id, match_id, user_id, muted_by_id, expires_at) VALUES (${randomUUID()}, ${message.matchId}, ${message.userId}, ${user.id}, CURRENT_TIMESTAMP + (${minutes} * INTERVAL '1 minute'))`;
  } else {
