...
export default async function TournamentsPage({ searchParams }: Props) {
  ...
  const filtered = selectedGame === "all" ? tournaments : tournaments.filter((t) => matchesGame(t.game, selectedGame));
  const live = filtered.filter((t) => t.status === "LIVE");
  const upcoming = filtered.filter((t) => t.status !== "LIVE");
  ...
  return (
    <main className="page-shell">
      <div className="page-container">
        <header className="mb-6 sm:mb-8">
          ...
          <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="page-subtitle">Find live and upcoming fighting game competitions by title.</p>
            </div>
        ...
        <section aria-label="Filter tournaments by game" className="surface-card mb-6 overflow-hidden p-2 sm:p-3">
          <div className="mb-2 px-1.5 sm:px-2">
            <span className="section-label">Games</span>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <Link href="/tournaments" className={`game-tab ${selectedGame === "all" ? "game-tab-active" : ""}`} aria-current={selectedGame === "all" ? "page" : undefined}>All</Link>
            {games.map((game) => {
              const active = matchesGame(game, selectedGame);
              return <Link key={game} href={`/tournaments?game=${encodeURIComponent(slugifyGame(game))}`} className={`game-tab ${active ? "game-tab-active" : ""}`} aria-current={active ? "page" : undefined}>{game}</Link>;
            })}
          </div>
        </section>
        ...
        {tournaments.length === 0 ? (
          ...
          <div className="empty-state">
            ...
            <h2 className="mt-2 font-display text-2xl uppercase tracking-wide">No active tournaments</h2>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-ink-faint">There are currently no live or upcoming {selectedLabel} tournaments.</p>
            <Link href="/tournaments" className="action-secondary mt-5">View all tournaments</Link>
          </div>
        ) : (
          <div className="space-y-8">
            {live.length > 0 && <TournamentSection title="Live now" tournaments={live} live />}
            {upcoming.length > 0 && <TournamentSection title={live.length > 0 ? "Upcoming & recent" : "Tournaments"} tournaments={upcoming} />}
          </div>
        )}
      </div>
    </main>
