        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="font-mono text-xs uppercase tracking-widest text-ink-faint">
              {tournament.game} · {tournament.sport}
        ...
        style={
          {
            ...
            ["--event-accent-2" as string]:
              tournament.organization.brandAccentColor ?? "#7c9cff",
          } as CSSProperties
        }
      >
        <section className="rounded-card border border-arena-700 bg-arena-900 p-4 sm:p-5">
          <div className="flex flex-wrap items-center gap-4">
            {tournament.organization.brandLogoUrl && (
              <img
                src={tournament.organization.brandLogoUrl}
                alt=""
            ...
            <div>
              ...
              <p className="text-sm text-ink-faint">
                {tournament.organization.tagline ??
                  "Official event broadcast hub"}
              </p>
            </div>
          </div>
        </section>
        ...
        {tournament.sponsors.length > 0 && (
          <section className="rounded-card border border-arena-700 bg-arena-900 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              {tournament.sponsors.map((sponsor) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-card border border-arena-700 px-4 py-2 hover:border-signal-live"
                >
                  {sponsor.logoUrl ? (
                    <img
                      src={sponsor.logoUrl}
        ...
        {tournament.teams.length > 0 && (
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {tournament.teams.slice(0, 12).map(({ team }) => (
                <Link
                  key={team.id}
                  href={`/teams/${team.id}`}
                ...
                >
                  ...
                  <p className="mt-2 font-display uppercase">{team.name}</p>
                </Link>
              ))}
            </div>
          </section>
