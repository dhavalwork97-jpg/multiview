...
async function loadDashboardData(user: NonNullable<Awaited<ReturnType<typeof getCurrentUser>>>): Promise<DashboardData> {
  try {
    ...
    const canCreateTournament = user.role === "ADMIN" || primaryRole === "OWNER" || primaryRole === "ADMIN";
    ...
    return {
      primaryRole: null,
      canCreateTournament: user.role === "ADMIN",
      tournaments: [],
      dataWarning: true,
    };
  }
      ...
      <div className="page-container">
        ...
        <header className="surface-card mb-8 p-4 sm:p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2"><SearchBar /><NotificationBell />{(canCreateTournament || primaryRole === "OPERATOR") && <Link href="/dashboard/team" className="action-secondary">Team</Link>}<Link href="/organization/settings" className="action-secondary">Branding</Link><Link href="/pricing" className="action-secondary">{isPremium(user) ? `Plans · ${trialDaysRemaining(user)}d` : "Plans"}</Link>{canCreateTournament && <Link href="/admin/tournaments/new" className="action-primary">Create tournament</Link>}</div>
          </div>
        </header>
