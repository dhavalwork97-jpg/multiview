...
export default async function PlayerPage({
  ...
  return (
    <main className="min-h-screen bg-arena-950 px-4 py-6 sm:px-6 sm:py-10">
      <div className="mx-auto max-w-6xl space-y-8">
        <section className="relative overflow-hidden rounded-[22px] border border-arena-600 bg-[radial-gradient(circle_at_82%_12%,rgba(95,45,255,.28),transparent_32%),radial-gradient(circle_at_12%_90%,rgba(0,207,255,.12),transparent_30%),linear-gradient(135deg,#071025,#030712)] p-5 shadow-elevated sm:p-8 lg:p-10">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <div className="grid grid-cols-3 gap-2 sm:min-w-[390px]">
              <div className="surface-quiet p-4"><p className="metric-label">Matches</p><p className="ds-number mt-2 text-3xl">{matches.length}</p></div>
              <div className="surface-quiet p-4"><p className="metric-label">Win rate</p><p className="ds-number mt-2 text-3xl text-signal-live">{winRate}%</p></div>
              <div className="surface-quiet p-4"><p className="metric-label">Record</p><p className="ds-number mt-2 text-3xl">{wins}-{losses}</p></div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-3 lg:grid-cols-[1.15fr_.85fr]">
          <div className="surface-card overflow-hidden p-5 sm:p-7">
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              <div className="surface-quiet p-5"><p className="metric-label">Victories</p><p className="ds-number mt-2 text-4xl">{wins}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-arena-800"><div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-cyan-400" style={{ width: `${winRate}%` }} /></div></div>
              <div className="surface-quiet p-5"><p className="metric-label">Losses</p><p className="ds-number mt-2 text-4xl">{losses}</p><p className="mt-2 text-xs text-ink-faint">recorded matches</p></div>
              <div className="surface-quiet p-5"><p className="metric-label">Recent form</p><div className="mt-3 flex gap-1.5">{matches.slice(0, 5).map((m) => <span key={m.id} className={`grid h-8 w-8 place-items-center rounded-full text-[10px] font-bold ${m.playerScore > m.opponentScore ? "bg-signal-live/15 text-signal-live" : "bg-rose-500/15 text-rose-300"}`}>{m.playerScore > m.opponentScore ? "W" : "L"}</span>)}</div></div>
            </div>
          </div>
