...
export function StationAssignmentBoard({ tournamentId }: { tournamentId: string }) {
  ...
  const [retryingEgress, setRetryingEgress] = useState<string | null>(null);

  // A station can show LIVE (its room chip is green) with no HLS/WebRTC
  // playback showing anywhere — normally the automatic retry on
  // track_published handles the "video track wasn't published yet" race
  // (see api/webhooks/livekit/route.ts), but a webhook delivery can still
  // get lost outright. This just asks the server to try starting egress
  // again for whatever match is currently queued/live on this station.
  async function retryEgress(stationId: string) {
    setRetryingEgress(stationId);
    try {
      await fetch(`/api/stations/${stationId}/retry-egress`, { method: "POST" });
      await refresh();
    } finally {
      setRetryingEgress(null);
    }
  }
        ...
        <ul className="space-y-2">
          {stations.map((s) => (
            ...
            >
              ...
              {s.status === "LIVE" && !s.playbackIdHls && (
                <p className="mt-2 rounded border border-signal-warn/50 bg-signal-warn/10 px-2 py-1 text-[11px] text-signal-warn">
                  Room is live but no playback source yet.{" "}
                  <button
                    type="button"
                    onClick={() => retryEgress(s.id)}
                    disabled={retryingEgress === s.id}
                    className="underline underline-offset-2 hover:text-signal-live disabled:opacity-50"
                  >
                    {retryingEgress === s.id ? "Retrying…" : "Retry stream"}
                  </button>
                </p>
              )}
