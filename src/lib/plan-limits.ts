...
export const PLAN_LIMITS: Record<PlanTier, { stations: number; tournaments: number; operators: number; analytics: boolean; branding: boolean; sponsors: boolean }> = {
  ...
  ENTERPRISE: { stations: 1000, tournaments: Number.POSITIVE_INFINITY, operators: 1000, analytics: true, branding: true, sponsors: true },
};

export function planAllows(plan: PlanTier, key: keyof typeof PLAN_LIMITS[PlanTier]) {
