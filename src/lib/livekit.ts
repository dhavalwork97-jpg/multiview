...
import { SegmentedFileOutput, S3Upload, TrackType } from "@livekit/protocol";

function requireLiveKitConfig() {
  const url = process.env.LIVEKIT_HTTP_URL;
  const apiKey = process.env.LIVEKIT_API_KEY;
  const apiSecret = process.env.LIVEKIT_API_SECRET;
  for (const [name, value] of [["LIVEKIT_HTTP_URL", url], ["LIVEKIT_API_KEY", apiKey], ["LIVEKIT_API_SECRET", apiSecret]] as const) {
    if (!value) {
      throw new Error(`Missing required env var ${name} — LiveKit is only required when a LiveKit route is actually used. Configure it before using LiveKit streaming.`);
    }
  }
...
export async function createStationIngress(stationId: string, label: string) {
  ...
  return { roomName, ingressId: ingress.ingressId, ingestUrl: ingress.url, streamKey: ingress.streamKey };
}
...
/** Store VOD as segmented HLS so full matches do not hit a single-object size limit. */
export async function startRoomEgress(roomName: string, matchId: string, stationId: string) {
  ...
  const publisher = participants.find((p) => p.tracks.some((t) => t.type === TrackType.VIDEO));
  if (!publisher) {
    throw new Error(`No published video track found for station=${stationId} room=${roomName} — encoder may not have finished connecting yet`);
  }
  ...
  const info = await getEgressClient().startTrackCompositeEgress(roomName, {
  ...
  return { egressId: info.egressId, hlsPlaylistKey: `${segmentPrefix}/index.m3u8`, mp4Key: null };
}
