...
async function youtubeWrite<T>(tournamentId: string, oidcToken: string | undefined, path: string, init: RequestInit, quotaUnits: number, operation: string) {
  ...
  try {
    return await youtubeRequest<T>(tournamentId, oidcToken, path, init);
  } catch (error) {
    if (isYouTubeQuotaError(error)) await markYouTubeQuotaBlocked();
    throw error;
  }
}

async function readStationStreamKey(value: string, oidcToken?: string) {
  if (value.startsWith("app-encrypted.v1.") || value.startsWith("gcp-kms.v1.")) {
    return decryptBroadcastSecret(value, oidcToken);
  }
  return value;
}
...
export async function ensureCustomerStationStream(stationId: string, oidcToken?: string) {
  ...
  if (station.youtubeStreamId && station.youtubeIngestUrl && station.streamKey) {
    return {
      ...
      ingestUrl: station.youtubeIngestUrl,
      streamKey: await readStationStreamKey(station.streamKey, oidcToken),
    };
  }
  ...
  if (claimed.count !== 1) {
    ...
    if (current?.youtubeStreamId && current.youtubeIngestUrl && current.streamKey) {
      return { streamId: current.youtubeStreamId, ingestUrl: current.youtubeIngestUrl, streamKey: await readStationStreamKey(current.streamKey, oidcToken) };
    }
    throw new Error(`Station ${station.label} is already preparing a YouTube stream. Wait a few seconds and try again.`);
  }

  try {
    const stream = await youtubeWrite<YouTubeStream>(
      station.tournamentId,
      oidcToken,
      "/liveStreams?part=snippet,cdn,contentDetails",
      {
        method: "POST",
        body: JSON.stringify({
          snippet: { title: `FGC Stream — ${station.label}` },
          cdn: { ingestionType: "rtmp", resolution: "variable", frameRate: "variable" },
          contentDetails: { isReusable: true },
        }),
      },
      YOUTUBE_QUOTA_UNITS.LIVE_STREAM_INSERT,
      "liveStreams.insert",
    );

    const info = stream.cdn?.ingestionInfo;
    if (!stream.id || !info?.ingestionAddress || !info.streamName) {
      throw new Error("YouTube created the stream but did not provide usable RTMP credentials");
    }
    ...
    await db.station.update({
      ...
      data: {
        ...
        youtubeIngestUrl: info.ingestionAddress,
...
export async function startCustomerStationBroadcast(matchId: string, oidcToken?: string) {
  ...
  try {
    ...
    const broadcast = await youtubeWrite<YouTubeBroadcast>(
      match.tournamentId,
      oidcToken,
      "/liveBroadcasts?part=snippet,status,contentDetails",
      {
        method: "POST",
        body: JSON.stringify({
          snippet: { title: `${match.tournament.name} — ${station.label}`, description: `${match.tournament.name} · ${station.label} · FGC Stream`, scheduledStartTime },
          status: { privacyStatus: "unlisted" },
          contentDetails: { enableAutoStart: true, enableAutoStop: false, enableDvr: true, recordFromStart: true },
        }),
      },
      YOUTUBE_QUOTA_UNITS.BROADCAST_INSERT,
    ...
    try {
      await youtubeWrite(
        match.tournamentId,
        oidcToken,
        `/liveBroadcasts/bind?id=${encodeURIComponent(broadcast.id)}&streamId=${encodeURIComponent(stream.streamId)}&part=id,snippet,contentDetails,status`,
        { method: "POST" },
        YOUTUBE_QUOTA_UNITS.BROADCAST_BIND,
        "liveBroadcasts.bind",
      );
    } catch (error) {
      try {
        await youtubeWrite(match.tournamentId, oidcToken, `/liveBroadcasts?id=${encodeURIComponent(broadcast.id)}`, { method: "DELETE" }, YOUTUBE_QUOTA_UNITS.BROADCAST_DELETE, "liveBroadcasts.delete");
      } catch (cleanupError) {
        console.error("[youtube customer] failed to clean up orphan broadcast", cleanupError);
      }
...
export async function stopCustomerStationBroadcast(stationId: string, oidcToken?: string) {
  ...
  await youtubeWrite(
    ...
    { method: "POST" },
    YOUTUBE_QUOTA_UNITS.BROADCAST_TRANSITION,
