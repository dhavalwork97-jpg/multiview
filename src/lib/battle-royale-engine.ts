...
function stageConfig(stage: { rules: unknown }): CompetitionStageConfig {
  const rules = stage.rules && typeof stage.rules === "object" && !Array.isArray(stage.rules)
    ...
    : {};
  return validateDynamicCompetition({
    version: 1,
...
async function calculateBattleRoyaleStandings(tx: Tx, stageId: string, cfg: CompetitionStageConfig): Promise<Standing[]> {
  const matches = await tx.match.findMany({
    ...
    include: { sides: { include: { participants: { include: { player: true } }, scoreEvents: true } } },
  ...
  for (const match of matches) {
    for (const side of match.sides) {
      ...
      const matchPoints = directPoints
        + (placement ? placementPoints(cfg, placement) : 0)
        + eliminations * cfg.scoring.eliminationPoints;
      ...
      map.set(participant.id, current);
    }
  }
...
async function createStageLobbies(tx: Tx, tournamentId: string, stageId: string, playerIds: string[]) {
  ...
  if (existing > 0 || ordered.length === 0) return [];
...
export async function generateBattleRoyaleCompetition(tx: Tx, tournamentId: string, config: DynamicCompetitionConfig) {
  ...
  return createStageLobbies(tx, tournamentId, stages[0].id, entrants.map((entry) => entry.playerId));
}
...
export async function progressBattleRoyaleStage(db: PrismaClient, matchId: string) {
  return db.$transaction(async (tx) => {
    ...
    let advanced: string[] = [];
    if (nextStage) {
      advanced = await createStageLobbies(tx, match.tournamentId, nextStage.id, standings.filter((row) => qualified.has(row.playerId)).map((row) => row.playerId));
