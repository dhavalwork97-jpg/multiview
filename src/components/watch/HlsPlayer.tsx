...
import { useWatchPartyMedia } from "@/components/social/WatchPartyContext";
...
export function HlsPlayer({
  ...
  const party = useWatchPartyMedia();
  const [error, setError] = useState(false);
  const [playing, setPlaying] = useState(false);

  useEffect(() => {
    if (!party) return;
    party.registerMedia(videoRef.current);
    return () => party.registerMedia(null);
  }, [party]);
  ...
  useEffect(() => {
    ...
    const scheduleRecovery = () => {
      ...
      retryTimerRef.current = setTimeout(() => {
        ...
        if (disposed) return;
        if (hlsRef.current) hlsRef.current.startLoad(); else setError(true);
      }, delay);
    };
    if (video.canPlayType("application/vnd.apple.mpegurl")) {
      video.src = src;
