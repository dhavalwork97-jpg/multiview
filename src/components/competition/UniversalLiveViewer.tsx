...
export function UniversalLiveViewer({
  ...
  const { state, refreshing } = useCompetitionViewerState(
    tournamentId,
    initialState,
  );
    ...
    <div className="space-y-8">
      <section className="rounded-card border border-arena-600 bg-arena-900 p-4 sm:p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] uppercase text-ink-faint">
              {state.live.matches.length} active
            </span>
          </div>
        </div>
        ...
        {!primaryLiveMatch ? (
          <div className="rounded-card border border-dashed border-arena-600 p-6 text-center">
            ...
            <p className="mt-1 text-sm text-ink-faint">
              Check upcoming matches and standings below.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <Link
              href={`/watch/${primaryLiveMatch.id}`}
              className="block rounded-card border border-signal-live/40 bg-arena-950 p-4 transition-colors hover:border-signal-live"
            >
              <div className="flex items-center justify-between gap-3">
                <span className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-signal-live">
                  <span className="h-1.5 w-1.5 animate-live-pulse rounded-full bg-signal-live" />
                  Live
                </span>
                <span className="font-mono text-[10px] uppercase text-ink-faint">
                  {primaryLiveMatch.station?.label ?? "Station"}
                </span>
              </div>

              <div className="mt-4">
                <CompetitionScoreboard sides={primaryLiveMatch.sides} />
              </div>

              {primaryLiveMatch.round && (
                <p className="mt-3 text-xs text-ink-faint">
                  {primaryLiveMatch.round} · Watch live
                </p>
              )}
            </Link>
            ...
            {state.live.matches.length > 1 && (
              <div className="grid gap-2 sm:grid-cols-2">
                {state.live.matches
                  .filter((match) => match.id !== primaryLiveMatch.id)
                  .map((match) => (
                    <Link
                      key={match.id}
                      href={`/watch/${match.id}`}
                      className="rounded-card border border-arena-700 bg-arena-950 p-3 hover:border-signal-live"
                    >
                      <CompetitionScoreboard sides={match.sides} compact />
                      <p className="mt-2 font-mono text-[10px] uppercase text-ink-faint">
                        {match.round ?? "Live match"} ·{" "}
                        {match.station?.label ?? "Station"}
                      </p>
                    </Link>
                  ))}
              </div>
      ...
      {state.upcoming.length > 0 && (
        <section>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {state.upcoming.slice(0, 6).map((match) => (
              <Link
                key={match.id}
                href={`/watch/${match.id}`}
                className="rounded-card border border-arena-700 bg-arena-900 p-4 hover:border-signal-live"
              >
                <CompetitionScoreboard sides={match.sides} compact />
                <p className="mt-3 font-mono text-[10px] uppercase text-ink-faint">
                  {match.round ?? "Upcoming match"}
                </p>
              </Link>
            ))}
          </div>
      ...
      {state.standings.length > 0 && (
        <section className="rounded-card border border-arena-600 bg-arena-900 p-4 sm:p-5">
          ...
          <div className="divide-y divide-arena-700">
            {state.standings.slice(0, 8).map((standing) => (
              <div
                key={standing.key}
                className="flex items-center justify-between gap-4 py-3"
              >
                <div className="flex min-w-0 items-center gap-3">
                  <span className="w-6 shrink-0 font-mono text-ink-faint">
                    {standing.rank}
                  </span>
      ...
      {state.recentResults.length > 0 && (
        <section className="rounded-card border border-arena-600 bg-arena-900 p-4 sm:p-5">
          ...
          <div className="divide-y divide-arena-700">
            {state.recentResults.slice(0, 8).map((match) => (
              <Link
                key={match.id}
                href={`/watch/${match.id}`}
                className="block py-3 hover:text-signal-live"
              >
                <CompetitionScoreboard sides={match.sides} compact />
                <p className="mt-1 font-mono text-[10px] uppercase text-ink-faint">
                  {match.round ?? "Completed match"} ·{" "}
                  {match.station?.label ?? "Station"}
                </p>
              </Link>
            ))}
