...
export async function SocialPulseWidgets() {
  try {
    ...
    const hotMatch = hottest[0]
      ? await db.match.findUnique({
          where: { id: hottest[0].matchId },
          select: { id: true, playerOne: { select: { gamertag: true } }, playerTwo: { select: { gamertag: true } } },
        })
      : null;
    ...
    console.error("FGC Stream social pulse load failed", error);
    return null;
  }
}
